...
export function getDecreaseError(p: {
  ...
  if (isContractAccount && isAddressZero(receiveToken?.address)) {
    return [`${receiveToken?.symbol} can not be sent to smart contract addresses. Select another token.`];
  }
  ...
  if (existingPosition) {
    ...
    if (
      ...
      return [`Leftover collateral below ${formatAmount(minCollateralUsd, USD_DECIMALS, 2)} USD`];
    }
  }
...
export function getEditCollateralError(p: {
  ...
  if (nextCollateralUsd && minCollateralUsd && position) {
    ...
    if (nextCollateralUsd.lt(minCollateralUsdForLeverage)) {
      return [`Min collateral: ${formatAmount(minCollateralUsdForLeverage, USD_DECIMALS, 2)} USD`];
    }
  }
