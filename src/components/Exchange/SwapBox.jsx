...
export default function SwapBox(props) {
  ...
  const orderOptionLabels = { [STOP]: `Trigger`, [MARKET]: `Market`, [LIMIT]: `Limit` };
    ...
    if (isShort) {
      ...
      if (fromTokenAddress !== shortCollateralAddress && fromAmount && fromAmount.gt(0)) {
        ...
        if (
          ...
          const nextUsdgAmount = fromTokenInfo.usdgAmount.add(usdgFromAmount);
          if (nextUsdgAmount.gt(fromTokenInfo.maxUsdgAmount)) {
            return [`Insufficient Liquidity`, ErrorDisplayType.Tooltip, ErrorCode.TokenPoolExceededShorts];
          }
        }
      }
      if (
  ...
  const ERROR_TOOLTIP_MSG = {
    [ErrorCode.InsufficientLiquiditySwap]: `Swap amount exceeds Available Liquidity.`,
          ...
          {showFromAndToSection && (
            <React.Fragment>
              ...
              <BuyInputSection
                ...
                topRightLabel={isSwap ? `Balance` : `Leverage`}
                ...
                <div className="Exchange-info-row">
                  {isExistingAndCollateralTokenDifferent ? (
                    <Tooltip
                      ...
                      handle={<span>Collateral In</span>}
                      renderContent={() => (
                        <span>
                          <span>
                          ...
                          >
                            <span>Switch to {existingCurrentIndexCollateralToken?.symbol} collateral.</span>
                          </div>
                        </span>
                      )}
                    />
                ...
                <div>
                  ...
                  {feesUsd && (
                    <FeesTooltip
                      ...
                      swapFee={swapFees}
                      titleText={swapFees && <span>{collateralToken.symbol} is required for collateral.</span>}
                    />
                  )}
                </div>
              </ExchangeInfoRow>
